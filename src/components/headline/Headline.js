	...
	return (
		<div className={styles.headline}>
			<div className={styles.backButton} onClick={() => navigate(-1)}>
				<svg viewBox="64 64 896 896" focusable="false" class="" data-icon="arrow-left" width="2em" height="2em" fill="hsla(230,40%,50%,1)" aria-hidden="true">
			...
			<div style={{display: 'flex', gap: '10px', height: '100%', alignItems: 'center'}}>
				<div className={styles.notification} >	
					<div className={styles.notificationIcon} onClick={() => {setNotification(prev => !prev)}} onBlur={() => {setNotification(false)}}>
						{  notificationShow ? 
							<NotificationFilled className={styles.notiIcon} style={{ fontSize: '23px', color: 'hsla(230,40%,50%,1)'}} onBlur={() => {setNotification(false)}}/> 
							: <NotificationOutlined className={styles.notiIcon} style={{ fontSize: '23px', color: 'hsla(230,40%,50%,1)' }} onBlur={() => {setNotification(false)}}/>
						}
					</div>
